...
void onCreate() {
  ...
  _control.throttle = MOTOR_MIN_MOTOR_MIN_POWER;
  _control.roll = 0;
  _control.yaw = 0;
  _control.pitch = 0;
}
...
void onMessageReceived(byte command, byte action, byte dataLength, byte* data) {
  ...
  switch (command) {
     case COMMAND_CONTROL:
        ...
        _control.roll = (int8_t) data[1];
        _control.yaw = (int8_t) data[2];
        _control.pitch = (int8_t) data[3];
      
        if (_control.throttle < 0) {
          _lod->d("Negative throttle detected, rounding to zero");
          _control.throttle = 0;
        }
