...
class EventHandler(tornadio2.SocketConnection):
	...
	def emit_all(self, event, type, throttle, pitch, roll, yaw):
		print "[{event}] {type}:: throttle: {throttle}, pitch: {pitch}, roll: {roll}, yaw: {yaw}".format(event=event, type=type, throttle=throttle, pitch=pitch, roll=roll, yaw=yaw)
		for connection in connections:
			print "connection: %s" % connection
			connection.emit(event, type, throttle, pitch, roll, yaw)
